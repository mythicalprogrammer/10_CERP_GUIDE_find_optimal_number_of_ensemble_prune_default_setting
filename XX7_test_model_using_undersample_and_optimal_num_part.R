...
# UNDERSAMPLE majority train data
train_majority_indicies <-
  sample(nrow(majority_train_data), nrow(minority_train_data))
train_majority <- majority_train_data[c(train_majority_indicies),]


majority_test_data <- myeloma_data[137:nrow(myeloma_data),]

test_data <- rbind(minority_test_data, majority_test_data)
train_data <- rbind(minority_train_data, train_majority)
